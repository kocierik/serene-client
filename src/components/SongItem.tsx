...
const SongItem = ({ songInfo, setSongDescription, fromYt }: Props) => {
  ...
  return (
    <div
      onClick={async () => {
        if (fromYt) await downloadSong(songInfo.path!);
        await getSong();
      }}
      className="relative group flex flex-col items-center justify-center rounded-md overflow-hidden gap-x-4 bg-base-300 cursor-pointer hover:bg-neutral-300/10 transition p-3"
    >
      <div className="relative flex items-center justify-center aspect-square w-full h-full rounded-md overflow-hidden">
        {songInfo.picture?.length != null && (
          <Image
            className="object-cover"
            sizes={'320'}
            src={
              songInfo.picture?.length > 100
                ? 'data:image/jpeg;base64,' + songInfo.picture
                : songInfo.picture!
            }
            alt="cover"
            fill
